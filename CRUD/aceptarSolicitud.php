...
$sqlIdUserSesion = "SELECT id FROM usuarios WHERE username = ?";
$stmtIdUserSesion = mysqli_prepare($conn, $sqlIdUserSesion);
mysqli_stmt_bind_param($stmtIdUserSesion, "s", $usuarioSesion);
mysqli_stmt_execute($stmtIdUserSesion);
mysqli_stmt_bind_result($stmtIdUserSesion, $resultadoIdUserSesion);
mysqli_stmt_fetch($stmtIdUserSesion);
mysqli_stmt_close($stmtIdUserSesion); // Liberar los resultados
    ...
    foreach ($usuarios as $usuario) {
        ...
        if ($resultadoIdUserPost) {
            ...
            if ($amistadExistente > 0) {
                ...
                if (mysqli_stmt_execute($stmtInsertarAmistad) && mysqli_stmt_execute($stmtEliminarSolicitud)) {
                ...
                mysqli_stmt_close($stmtInsertarAmistad);
                mysqli_stmt_close($stmtEliminarSolicitud);
            }
        } else {
